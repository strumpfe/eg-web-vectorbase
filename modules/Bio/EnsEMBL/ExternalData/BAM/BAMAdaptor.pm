...
## VB - ftp hack
# the regular VB ftp server doesn't support PASV inside the firewall
# so we rewrite the url to use the internal ftp server
  if (my $prefix = $SiteDefs::VECTORBASE_FTP_PREFIX) {
    $url =~ s/ftp\.vectorbase\.org/$prefix.vectorbase.org/;
  }
## /VB
  warn $url;
